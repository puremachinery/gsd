...
const
  ...
  CodexAgentsSectionStart = "<!-- GSD:AGENTS START -->"
  CodexAgentsSectionEnd = "<!-- GSD:AGENTS END -->"

proc resolveToolDir(gsdDir: string, p: Platform): Option[string] =
  ## Resolve tool directory from gsdDir and install metadata.
  ## Falls back to path inference for local/global legacy compatibility.
  let cfg = loadConfig(gsdDir)
  if cfg.isSome:
    case cfg.get().installType
    of itLocal:
      return some(platform.getLocalConfigDir(p))
    of itGlobal, itCustom:
      return some(platform.getGlobalConfigDir(p))

  let inferred = inferInstallType(gsdDir)
  case inferred
  of itLocal:
    return some(platform.getLocalConfigDir(p))
  of itGlobal:
    return some(platform.getGlobalConfigDir(p))
  of itCustom:
    stderr.writeLine "Error: Unable to determine install scope for ", gsdDir, "."
    stderr.writeLine "Provide a valid .gsd directory containing gsd-config.json."
    return none(string)
...
proc renderCodexAgentsSection*(agentFiles: seq[string], installType,
    gsdDir: string): string =
  ## Build the managed GSD section that is inserted into AGENTS.md
  var content = CodexAgentsSectionStart & "\n"
  content.add "# GSD Agents\n\n"
  content.add "This section is auto-generated by GSD installer.\n"
  content.add "Edits inside this section will be overwritten.\n\n"
  ...
  for agentPath in sortedFiles:
    if fileExists(agentPath):
      ...
      content.add rewritten.strip() & "\n\n"

  content.add CodexAgentsSectionEnd & "\n"
  return content

proc stripCodexAgentsSection*(content: string): string =
  ## Remove the managed GSD section from AGENTS.md while preserving user content.
  var resultLines: seq[string] = @[]
  var inManagedSection = false

  for line in content.splitLines():
    let stripped = line.strip()
    if not inManagedSection and stripped == CodexAgentsSectionStart:
      inManagedSection = true
      continue
    if inManagedSection:
      if stripped == CodexAgentsSectionEnd:
...
proc mergeCodexAgentsMd*(existingContent, gsdSection: string): string =
  ## Merge managed GSD section into AGENTS.md, preserving user-authored content.
  let base = stripCodexAgentsSection(existingContent)
  if base.strip().len == 0:
    return gsdSection
  ...
  if not merged.endsWith("\n\n"):
    merged.add("\n")
  merged.add(gsdSection)
  return merged

proc generateAgentsMd*(agentFiles: seq[string], destPath: string, installType,
    gsdDir: string): bool =
  ## Update Codex AGENTS.md with a managed GSD section while preserving user content.
  try:
    let existing = if fileExists(destPath): readFile(destPath) else: ""
    let gsdSection = renderCodexAgentsSection(agentFiles, installType, gsdDir)
    let merged = mergeCodexAgentsMd(existing, gsdSection)
...
proc uninstallCodex*(gsdDir: string, verbose: bool, dryRun: bool = false): bool =
  ...
  let toolDirOpt = resolveToolDir(gsdDir, pCodexCli)
  if toolDirOpt.isNone:
    return false
  ...
  if fileExists(agentsMdFile):
    try:
      let existingContent = readFile(agentsMdFile)
      let cleanedContent = stripCodexAgentsSection(existingContent)
      if cleanedContent.strip().len == 0:
...
proc uninstall*(gsdDir: string, verbose: bool, p: Platform = pClaudeCode,
  ...
  if fileExists(settingsPath):
    ...
    # Remove only GSD hooks while preserving custom hooks (including mixed entries)
    if settings.hasKey("hooks"):
      settings["hooks"] = mergeHooks(settings, newJObject())
...
proc isLegacyLocalInstall(legacy: InstalledConfig): bool =
  ## Infer whether a legacy tool-dir install is local or global.
  let cfg = loadConfig(legacy.dir)
  if cfg.isSome:
    case cfg.get().installType
    of itLocal:
      return true
...
proc migrateLegacyGroup(legacyConfigs: seq[InstalledConfig], gsdDir: string,
  ...
  # Collect all platforms in this scope
  ...
  # Clean up old files from legacy tool dirs in this scope
  for legacy in legacyConfigs:
...
proc migrateLegacyInstall*(legacyConfigs: seq[InstalledConfig], verbose: bool): bool =
  ...
  var localLegacy: seq[InstalledConfig] = @[]
  var globalLegacy: seq[InstalledConfig] = @[]

  for legacy in legacyConfigs:
    if isLegacyLocalInstall(legacy):
      localLegacy.add(legacy)
    else:
      globalLegacy.add(legacy)

  if localLegacy.len > 0:
    if not migrateLegacyGroup(localLegacy, platform.getLocalGsdDir(), itLocal, verbose):
      return false

  if globalLegacy.len > 0:
    if not migrateLegacyGroup(globalLegacy, platform.getGlobalGsdDir(), itGlobal, verbose):
      return false
