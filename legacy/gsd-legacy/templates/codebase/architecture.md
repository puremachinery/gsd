...
- Used by: [What uses it: e.g., "service handlers"]
...
**[Entry Point]:**
- Location: [Brief: e.g., "src/index.ext", "API Gateway triggers"]
...
- Location: `src/commands/*.ext`
- Depends on: Service layer for business logic
- Used by: CLI entry point (`src/index.ext`)

**Service Layer:**
- Purpose: Core business logic
- Contains: FileService, TemplateService, InstallService
- Location: `src/services/*.ext`
...
3. Command handler invoked (`src/commands/new-project.ext`)
4. Handler calls service methods (`src/services/project.ext` → `create()`)
...
- Examples: `src/services/file.ext`, `src/services/template.ext`, `src/services/project.ext`
- Pattern: Singleton-like (imported as modules, not instantiated)

**Command:**
- Purpose: CLI command definition
- Examples: `src/commands/new-project.ext`, `src/commands/plan-phase.ext`
- Pattern: Commander.ext command registration
...
Include file paths as concrete examples of abstractions. Use backtick formatting: `src/services/user.ext`. This makes the architecture document actionable for assistant when planning.
