...
**Runner:**
- [Framework: e.g., "TestRunner 29.x", "TestRunner 1.x"]
- [Config: e.g., "test.config.ext in project root"]
...
**Run Commands:**
```bash
[e.g., "run-tests" or "test-command"]              # Run all tests
[e.g., "run-tests --watch"]                     # Watch mode
[e.g., "run-tests path/to/file.test.ext"]       # Single file
[e.g., "run-tests --coverage"]                   # Coverage report
```

## Test File Organization

**Location:**
- [Pattern: e.g., "*.test.ext alongside source files"]
- [Alternative: e.g., "__tests__/ directory" or "separate tests/ tree"]

**Naming:**
- [Unit tests: e.g., "module-name.test.ext"]
- [Integration: e.g., "feature-name.integration.test.ext"]
- [E2E: e.g., "user-flow.e2e.test.ext"]
...
**Framework:**
- [Tool: e.g., "TestRunner built-in mocking", "TestRunner mock", "mocking-lib"]
- [Import mocking: e.g., "mock.mock() at top of file"]
...
mock.mock('./external-service', () => ({
  service callData: mock.fn()
}));

// Mock in test
const mockFetch = mock.mocked(service callData);
...
- [e.g., "Time/dates (use mock.useFakeTimers)"]
- [e.g., "Network calls (use mock service call)"]
...
- [Exclusions: e.g., "exclude *.test.ext, config files"]

**View Coverage:**
```bash
[e.g., "run-tests --coverage"]
...
**Snapshot Testing:**
- [Usage: e.g., "for UIFramework modules only" or "not used"]
...
**Runner:**
- TestRunner 1.0.4
- Config: test.config.ext in project root

**Assertion Library:**
- TestRunner built-in expect
...
run-tests path/to/file.test.ext     # Single file
run-tests --coverage                 # Coverage report
```
...
**Naming:**
- unit-name.test.ext for all tests
...
**Suite Organization:**
```text
import { describe, it, expect, beforeEach, afterEach, mock } from 'test-runner';
...
- Use afterEach to restore mocks: mock.restoreAllMocks()
...
**Framework:**
- TestRunner built-in mocking (mock)
- Module mocking via mock.mock() at top of test file

**Patterns:**
```text
import { mock } from 'test-runner';
import { externalFunction } from './external';

// Mock module
mock.mock('./external', () => ({
  externalFunction: mock.fn()
}));

describe('test suite', () => {
  it('mocks function', () => {
    const mockFn = mock.mocked(externalFunction);
...
**What to Mock:**
- File system operations (fs-lib)
- Child process execution (process-exec)
...
**Configuration:**
- TestRunner coverage via c8 (built-in)
- Excludes: *.test.ext, bin/install.ext, config files
...
- Mock all external dependencies (filesystem, process exec)
- Fast: each test <100ms
- Examples: parser.test.ext, validator.test.ext

**Integration Tests:**
- Test multiple modules together
- Mock only external boundaries (file system, process)
- Examples: install-service.test.ext (tests service + parser)
...
```text
import { mock } from 'test-runner';
import * as fs from 'fs-lib';

mock.mock('fs-lib');

it('mocks file system', () => {
  mock.mocked(fs.readFile).mockResolvedValue('file content');
  // test code
});
```
...
- Check project.manifest scripts for test commands
- Find test config file (test.config.ext, test.config.ext)
...
**Analysis approach:**
- Check project.manifest for test framework and scripts
