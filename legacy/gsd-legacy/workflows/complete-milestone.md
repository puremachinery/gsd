...
find . -name "*.swift" -o -name "*.ext" -o -name "*.py" | xargs wc -l 2>/dev/null
...
Shipped v1.0 with 2,400 LOC TypedLanguage.
Tech stack: WebFramework, DatabaseProvider, Canvas API.
...
2. Read `~/.claude/gsd/templates/milestone-archive.md` template
