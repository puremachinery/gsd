...
How to verify:
  1. Check src/db/schema.ext for correct types
...
**Example: HostingProvider deployment hits auth error**

```
Task 3: Deploy to HostingProvider
...
Task: Authenticate HostingProvider CLI
...
- **Found during:** Task 4 (Follow/unfollow service implementation)
...
- **Files modified:** src/models/User.ext, migrations/003_fix_email_unique.sql
...
- **Files modified:** src/middleware/auth.ext, src/middleware/auth.test.ext
...
- Install minimal test framework (TestRunner, pytest, Go testing, etc.)
...
|------|-------------|---------|
| `feat` | New feature, endpoint, module, functionality | feat(08-02): create user registration endpoint |
...
| `docs` | Documentation changes | docs(08-02): add service endpoint documentation |
...
**Critical: assistant automates everything with CLI/API before checkpoints.** Checkpoints are for verification and decisions, not manual work.
...
```
I automated: [what assistant already did via CLI/API]
...
See ~/.claude/gsd/references/checkpoints.md for complete checkpoint guidance.
</step>
...
|------|------|--------|-------|
| 1 | Initialize WebFramework 15 project | d6fe73f | project.manifest, langconfig.json, modules/ |

### Current Task

**Task 2:** Initialize BackendService backend
**Status:** blocked
**Blocked by:** BackendService CLI authentication required

### Checkpoint Details

**Automation attempted:**
Ran `backend-service dev` to initialize BackendService backend

**Error encountered:**
"Error: Not authenticated. Run `backend-service login` first."

**What you need to do:**
1. Run: `backend-service login`
2. Complete browser authentication
3. Run: `backend-service dev`
4. Create project when prompted

**I'll verify after:**
`cat .env.local | grep CONVEX` returns the BackendService URL

### Awaiting

Type "done" when BackendService is authenticated and project created.
```
...
Create `.planning/phases/XX-name/{phase}-USER-SETUP.md` using template from `~/.claude/gsd/templates/user-setup.md`.
...
2. For each service, generate sections:
   ...
   - Control Panel Configuration steps (from `dashboard_config`, if present)
...
|--------|----------|--------|--------|
| [ ] | `PAYMENTS_API_KEY` | PaymentsProvider Control Panel → Developers → service keys → Secret key | `.env.local` |
| [ ] | `PAYMENTS_WEBHOOK_SECRET` | PaymentsProvider Control Panel → Developers → Webhooks → Signing secret | `.env.local` |

## Control Panel Configuration

- [ ] **Create callback endpoint**
  - Location: PaymentsProvider Control Panel → Developers → Webhooks → Add endpoint
  - Details: URL: https://[your-domain]/callbacks/payments-provider, Events: checkout.session.completed
...
Use ~/.claude/gsd/templates/summary.md for structure.
...
3. **Tech tracking:**
   - tech-stack.added: New libraries from project.manifest changes or requirements
...
For commit message conventions, see ~/.claude/gsd/references/git-integration.md
</step>
...
| project.manifest deps changed | STACK.md: Add/remove from dependencies list |
| New file pattern (e.g., first .test.ext) | CONVENTIONS.md: Note new pattern |
| New external service client | INTEGRATIONS.md: Add service entry with file path |
