...
With diagnosis: "Comment doesn't refresh" → "lifecycle hook missing dependency" → precise fix
</core_principle>
...
- truth: "Comment appears immediately after submission"
  ...
  severity: major
  test: 2
  root_cause: "lifecycle hook in CommentList.ext missing commentCount dependency"
  artifacts:
    - path: "src/modules/CommentList.ext"
      issue: "lifecycle hook missing dependency"
  missing:
    - "Add commentCount to lifecycle hook dependency array"
...
|-------------|------------|-------|
| Comment appears immediately | lifecycle hook missing dependency | CommentList.ext |
| Reply button positioned correctly | CSS flex order incorrect | ReplyButton.ext |
| Delete removes comment | service missing auth header | service/comments.ext |
